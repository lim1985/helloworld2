<template>
  ...
  <div class="wrapper" @click.stop>
    ...
    <div v-if="isAdmin">
      ...
      <van-panel
        :title="
          `审核员：${AdminUserInfo.DepartmentName}-${AdminUserInfo['U.UserName']}`
        "
      >
        <van-form validate-first @submit="submitKako">
          <!-- // @failed="onFailed" -->
    ...
    <van-dialog
      v-model="showchooicePersonTypePanle"
      @confirm="goKDQr"
      title="标题"
      show-cancel-button
    >
...
import Vue from "vue";
import { kakoAdd, kakoselect } from "@/api/index";
...
export default {
  ...
  data() {
    this.kakoNameRules = [{ required: true, message: "卡口地址不可为空" }];
    this.cityRules = [{ required: true, message: "请选择卡口地址" }];
  ...
  methods: {
    ...
    onConfirm_KD(val) {
      ...
      Vue.ls.set(
        Kako_address,
        `${this.Report.SQaddress + "-" + this.Report.kkname}`,
        15 * 24 * 60 * 60 * 1000
      ); //设置本地缓存
      // this.isshowKD=false
      // this.kako_value
    ...
    goKDQr() {
      ...
      setTimeout(() => {
        vm.$router.push({
          path: "/qr",
          query: { kokaID: vm.Qr.kokaID, personType: this.personType }
        });
      }, 1000);
