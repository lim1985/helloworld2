<template>
  ...
  <van-field v-model="text" placeholder="请输入用户名" />
    ...
    <navigations
      ...
      :kak_List="xzsqoptions"
      @ok="backReport"
      :AdminUserInfo="adminInfo"
      @check="check_LS_User"
      ref="nav"
      :showAdmin="isAdmin"
      :show="isnav"
...
湖北暴露史人员 -->
          ...
          <van-button :disabled="buttondisabled" type="primary">{{
            submit
          }}</van-button>
        </div>
      </van-panel>
...
export default {
  ...
  data() {
    ...
    this.SQ_Rules = [{ required: true, message: "请选择街道/社区或村" }];
    this.cityRules = [{ required: true, message: "请选择户籍城市" }];
    this.phoneRules = [
  ...
  methods: {
      ...
      setTimeout(() => {
        getUserInfo({ ReportId: id, tel: tel }).then(res => {
          ...
          if (data.code != -1) {
            ...
            this.Report.D_class_LkwhDateString =
              data.D_class_LkwhDate == null
                ? ""
                : this.$moment(data.D_class_LkwhDate).format("YYYY-MM-DD");
            ...
            this.Report.personTitle =
              data.personType == 0 ? "本地居民" : "返邵来邵人员";
    ...
    async sumitreport() {
      this.Report.inputTime = this.$moment().format(
    ...
    isLkhb(val) {
      ...
      if (val == "1") {
        ...
        if (
          !this.Report.D_class_LkwhDateString ||
          this.Report.D_class_LkwhDateString == "Invalid date"
        ) {
    ...
    onConfirm(FJ_value) {
      ...
      this.Report.FJquxian = FJ_value[2];
      0;
