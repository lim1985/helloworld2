...
<van-row>
  <van-col span="24">
    <div class="text_1">     
      当天({{todayTimes}})，登记总数{{toDayCountsdata.AllCount}}人，其中A类{{ toDayCountsdata.AclassCount}}人，B类{{ toDayCountsdata.BclassCount}}人，
      C类{{ toDayCountsdata.CclassCount}}人， D类{{ toDayCountsdata.DclassCount}}人。
 ...
 <van-row>
  <van-col span="24">
    <div class="text_1">     
     昨天({{yesToday}}),登记总数{{yesTeDayCountsdata.AllCount}}人，其中A类{{ yesTeDayCountsdata.AclassCount}}人，B类{{ yesTeDayCountsdata.BclassCount}}人，
      C类{{ yesTeDayCountsdata.CclassCount}}人， D类{{ yesTeDayCountsdata.DclassCount}}人。
      <!-- (其中：返邵来邵人员{{toDayCountsdata.FsPersonCount}}人，本地居民{{toDayCountsdata.localPersonCount}}人) -->
    </div> 
     </van-col>
 </van-row>
...
dCount:343
jiedao:null -->
 <van-row class="mytr" v-for="(item,index) in FS_Local_arr" :key="index">
  <van-col class="mytd"  span="7">
    <div class="title_2">
...
{{item.dCount==false?'0':item.dCount}}人
  ...
  <van-divider :style="{ color: '#1989fa','margin-top': '0px', borderColor: '#ccc', }">
...
<van-row>
  <van-col span="24">
    <div class="text_1">     
      当天({{todayTimes}})，登记总数{{localtoDayCountsdata.AllCount}}人，其中A类{{ localtoDayCountsdata.AclassCount}}人，B类{{ localtoDayCountsdata.BclassCount}}人，
      C类{{ localtoDayCountsdata.CclassCount}}人。
      <!-- (其中：返邵来邵人员{{toDayCountsdata.FsPersonCount}}人，本地居民{{toDayCountsdata.localPersonCount}}人) -->
    </div> 
     </van-col>
 </van-row>
 ...
 <van-row>
  <van-col span="24">
    <div class="text_1">     
     昨天({{yesToday}}),登记总数{{localyesTeDayCountsdata.AllCount}}人，其中A类{{ localyesTeDayCountsdata.AclassCount}}人，B类{{ localyesTeDayCountsdata.BclassCount}}人，
      C类{{ localyesTeDayCountsdata.CclassCount}}人。
      <!-- (其中：返邵来邵人员{{toDayCountsdata.FsPersonCount}}人，本地居民{{toDayCountsdata.localPersonCount}}人) -->
    </div> 
     </van-col>
 </van-row>
...
总数
 ...
 <van-row class="mytr" v-for="(item,index) in Local_arr" :key="index+100">
  <van-col class="mytd"  span="7">
    <div class="title_2">
{{item.jiedao==null?'未分类':item.jiedao}}
    </div>
...
<van-row class="lim_row2">
          <van-col span="11">
          <div class="title_3">
          <span>重点观察：<b style="color：red;">{{Countsdata.ZDGZ_Count}}</b>人</span>
        </div>
       </van-col> 
       <van-col  span="11" >
         <div class="title_3">
...
<van-row v-for="(item,index) in kakoInfoAndAll.kakolist" :key="index" >
  <van-col span="24">
   <div class="title_1">
     <van-collapse-item  :title="`${item.text},卡口数:${item.count}个`" :name="index">
        <van-row v-for="(SqItem,index) in item.children" :key="index"> 
        <van-col span="24">
   <div class="text_4"><span>{{index+1}}、社区：{{ SqItem.text }}</span></div>
    </van-col>  
       <van-row v-for="(kdItem,index) in SqItem.kd_list" :key="index"> 
             <van-col span="24">
   <div class="text_3"><span>卡口：{{ kdItem.kakName }}</span></div>
    </van-col>         
      </van-row>
       <van-divider :style="{ color: '#EBEEF5', borderColor: '#EBEEF5', padding: '0 0px' }">
...
<van-row>
  ...
  <van-row class="lim_row" v-for="(item,index) in jdcountList" :key="index">
        <van-col span="18">
          <div class="title_5">
          <span> {{index+1+`、`}} {{ item.jiedao==null?'未明确人员':item.jiedao}} {{ item.counts}}人</span>
        </div>
       </van-col>   
       <!-- <van-col  span="6">
        ...
        <van-col  span="6">      
          <div class="title_2 " style="float:right;padding-right: 10px;">
              <span>
                <router-link :to="{name:'PersonList',query:{jdId:item.JdId==null?-1:item.JdId}}"> 详细</router-link>
               </span>
            </div>
        <!-- <van-icon style="font-size:16px;" name="user-o">{{item.counts}}人</van-icon > -->
       </van-col>               
...
export default {
  ...
  methods: {
    ...
    var result = [], // 用来收集结果
    json = {}; // 用来合并重复项
    function cleanData(data){// 清理整合数据项，转换为 {jiedao:'', 'A_class或者B_class': n}
        var ret = {};
        for(var k in data){
            if(data[k] === true){
              console.log(data[k])
    ...
    list.forEach(item=>{// 遍历原始数据
        item = cleanData(item);
        var jd = item[key];// 取得合并的依据字段名
        if(jd in json){// 如果收集过数据，尝试在第一次收集的项上做合并
            var obj = json[jd];// 拿出上次收集的项
            for(var k in item){// 遍历本次项的字段
...
//       Be15Dayafter:'',
      ...
      // }
       ...
       let yestoday=this.$moment().subtract('days',1).format('MM-DD')//昨天
     ...
     // this.$moment().format("MM-DD");
      ...
      //  console.log(kakoAllCounts);
       ...
       //15天前到当天的数据
         ...
         this.localDay15beforCountsdata.AllCount= this.localDay15beforCountsdata.AllCount+v.counts
         if(v.personType==1)
         {
         this.localDay15beforCountsdata.FsPersonCount= this.localDay15beforCountsdata.FsPersonCount+v.counts
         }
         else
         {
         this.localDay15beforCountsdata.localPersonCount= this.localDay15beforCountsdata.localPersonCount+v.counts
         }
         switch (v.status) {
           case 66:
            this.localDay15beforCountsdata.QXGZ_Count= this.localDay15beforCountsdata.QXGZ_Count+v.counts
             break;
           case 99:
             this.localDay15beforCountsdata.ZDGZ_Count= this.localDay15beforCountsdata.ZDGZ_Count+v.counts
             break;         
           default:
             break;
         }
