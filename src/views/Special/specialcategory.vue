...
<transition name="van-fade">
  ...
  background="#1989fa" v-model="searchvalue" placeholder="请输入搜索关键词" />
          </van-col>
           <van-col span="24">
         ...
         <van-row>
          <van-col span="24">
             <h2>专题：{{  specialTitle }}</h2>
          </van-col>
      </van-row>
      <van-row class="zhuanlan">
        <template v-for="(item,index) in speciasItem">
          <div :key="index">
        <van-col class="zhuanlanItem" @click="viewspecial(item.SpecialID)" span="7">{{item.SpecialName}}</van-col>
     ...
     <van-row>
          <van-col span="24">
                <van-swipe @change="onChange" class="my-swipe" width="350" style="height: 180px;" :show-indicators="false" indicator-color="white">
        <van-swipe-item v-for="(item, index) in hotlist" :key="index">
            <template v-if="index<=3">
                 <div class="tophot">
                    <div class="hot_icon"><van-icon name="fire" /> </div>
            <div class="hot">热点精选</div>
            </div>
        
            <div class="hot_title">{{item.Title}}</div>
               <van-image @click="viewContent(item)" lazy-load  width="97%" height="9em"  fit="cover"  :src="`${item.DefaultPicUrl}`" />   
                 ...
                 <van-row>
                    <van-col span="16">
                      ...
                      <van-col class="imglists" span="7">
                           <van-image lazy-load width="100%" height="80"  fit="cover"  :src="`${item.DefaultPicUrl}`" />   
...
export default {
  ...
  methods: {
      ...
      {
            let alllist=[]
            let specialcategoryid=this.$route.params.categoryId
            let result=await getAllspecialID({specialcategoryID:specialcategoryid})
            console.log(result)
            this.speciasItem=result.data.specialObj.list
            this.specialTitle=result.data.specialObj.SpecialCategoryName
            let specialIDs=result.data.specialObj.specialIds;
              console.log(specialIDs)
            for(let x=0;x<specialIDs.length;x++)
            {            
              let arr=await this.initdata(specialIDs[x])            
              alllist.push(arr.data.result);
            }

            for(let x=0;x<alllist.length;x++)
            {
              if(alllist[x].hotlist.length>0)
              {
                console.log(alllist[x].hotlist)
                this.hotlist=this.hotlist.concat(alllist[x].hotlist)
              }
              if(alllist[x].imageContent.length>0)
              {
                this.imagespecialList=this.imagespecialList.concat(alllist[x].imageContent)
                  console.log(this.imagespecialList);
              }
              if(alllist[x].textContent.length>0)
              {
                this.textspecialList=this.textspecialList.concat(alllist[x].textContent)
                console.log(this.textspecialList);
              }
            }
            console.log(alllist);
            console.log(`textspecialList`,this.textspecialList)
            console.log(this.imagespecialList)
      ...
      viewspecial(specialId)
      {
         this.$router.push({path:`/Special/${specialId}`})
      },
     async initdata(specialid)
      {
          ...
          // if(this.speciallist.data.code==100)
          // {
