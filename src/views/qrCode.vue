...
<!-- <van-icon name="arrow-left" /> -->
    <div>请微信扫码进行提交您的数据</div>
     <div><h3>当前卡口：{{kako_address }}</h3></div>
          <div><h3>人员类别：{{personTypeTitle }}</h3></div>
...
<script>
  ...
  vm.$router.push({path:'/'})
      ...
      if(!tel)
      {
         this.$router.push({path:'/#/report' ,query: { redirect: this.$route.fullPath }})  
         return false   
      }
      return true
    },
      initQR() {
        ...
        params.personType==0?this.personTypeTitle='本地居民':this.personTypeTitle='返邵来邵人员'
        // let tel=Vue.ls.get(Tel)
          vm.qrCodeString = window.location.protocol + '//' +  window.location.host +'/report/#/report?kokaID='+params.kokaID+'&personType='+params.personType
