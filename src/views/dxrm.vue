<template>
  <div v-if="isRealUrl" class="hello">
    ...
    <div class="dengji_panel">
      ...
      <van-panel
        class="Isperson_panel"
        v-if="wasSubmit"
        :title="`${form.DepName}:已登记安装 ${DepCount} 人`"
      >
        <div class="my_panel">
          <!-- <van-row gutter="10">
...
<van-row gutter="10">
  ...
  <van-col span="16">{{ form.DepName }}</van-col>
          ...
          <van-row class="bottom_button" gutter="10">
            <van-col class="text_result" span="24"
              >温馨提示：如有疑问，请致电<a href="tel:07395396272">
                0739-5396272
              </a></van-col
            >
  ...
  <van-col v-if="status==1" span="24">
     <van-button round block type="info" native-type="submit">
      ...
      <van-row
        v-if="isPerson"
        gutter="10"
        style="text-align: right;margin: 6px 20px;"
      >
        <van-col span="24">
  ...
  methods: {
    isFormUrl() {
      ...
      if (_fromUrl == null) {
        window.location.href =
          "https://daxiang-wap.rednet.cn/app/index.html?contentId=6966421&contentType=1";
      } else {
        this.isRealUrl = true;
      }
    },
    ...
    async getSms() {
      ...
      setTimeout(() => {
        GetVerificatCodeAdSmssecret({
          contents:
            "输入验证码后请注明本人姓名及所在单位，系统会自动统计各单位关于大祥融媒APP安装数量，感谢您的支持！",
          mobile: this.Report.tel
        }).then(res => {
