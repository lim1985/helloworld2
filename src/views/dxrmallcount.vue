...
/>
  ...
  <van-col style="text-align:left;padding:14px 5px 5px 5px;" span="20">
    <!-- <van-notice-bar text="温馨提醒:大祥区雨溪公墓实行按日分时段限流预约祭扫，可通过“云上清明”微信公众号或拨打电话0739-5300667预约或咨询。3月28日至４月12日，每个墓穴每天入园祭扫人员不超过３人，祭扫时间不超过１小时，未预约不得入园祭扫。" left-icon="volume-o" />
     -->
  <span style="font-size: 16px;padding: 10px;">你好，今天是{{ toDay }}</span>
  </van-col> 
   <van-col style="padding:10px 5px 5px 5px;" span="4">
    <!-- <van-notice-bar text="温馨提醒:大祥区雨溪公墓实行按日分时段限流预约祭扫，可通过“云上清明”微信公众号或拨打电话0739-5300667预约或咨询。3月28日至４月12日，每个墓穴每天入园祭扫人员不超过３人，祭扫时间不超过１小时，未预约不得入园祭扫。" left-icon="volume-o" />
     -->
  <span @click="showcalendar" style="font-size: 22px;padding: 10px;"><van-icon name="setting-o" /> </span>
  </van-col> 
</van-row>
  <van-row>
  ...
  <van-col style="padding:0px 5px 5px 5px;" span="24">
    <!-- <van-notice-bar text="温馨提醒:大祥区雨溪公墓实行按日分时段限流预约祭扫，可通过“云上清明”微信公众号或拨打电话0739-5300667预约或咨询。3月28日至４月12日，每个墓穴每天入园祭扫人员不超过３人，祭扫时间不超过１小时，未预约不得入园祭扫。" left-icon="volume-o" />
     -->
  <van-button @click="copytoClipboard" type="info">点击复制上述内容</van-button>
  ...
  <van-collapse v-model="activeName" accordion>
    <template v-for="(item,index) in datalist">
      <div :key="index">     
         <van-collapse-item style="font-size:20px" :title="`${item.keyname}稿件${item.count}条`" :name="index+1">
...
export default {
  ...
  {     
    return {
      datalist:[],
      activeName:1,
      minDate: new Date(this.$moment().startOf('year').format('YYYY'), 0, 1),
      maxDate: new Date(this.$moment().startOf('year').format('YYYY'), 11, this.$moment().endOf('year').format('DD')),
      toDay:'',
      active:1,
      resResult:'',
      show:false
  ...
  methods: {
    ...
    init(){
      let date={}
      let afterDayTime=this.$moment().subtract(1, 'days').format('YYYY-MM-DD 23:59:00'); /*前一天的时间*/
      let after2DayTime=this.$moment().subtract(2, 'days').format('YYYY-MM-DD 23:59:00'); /*前一天的时间*/
   ...
   async onConfirm(e){
    ...
    copytoClipboard(){  
      let that=this
        that.$copyText(this.resResult).then(function (e) {
    ...
    async initdata(date)
    {
      ...
      let currentMM=this.$moment().endOf('year').format('DD')
      console.log(currentMM)
    
      // let now=this.$moment().format('YYYY-MM-DD HH:mm:ss' )
      let afterDayTime=this.$moment().subtract(1, 'days').format('YYYY-MM-DD 23:59:00'); /*前一天的时间*/
      let after2DayTime=this.$moment().subtract(2, 'days').format('YYYY-MM-DD 23:59:00'); /*前一天的时间*/
      let afterDay=this.$moment().subtract(1, 'days').format('YYYY年MM月DD日');
      ...
      if(date.selectd)
      {
         ...
         let _data=result.data.objresult
         this.resResult=`【大祥区融媒体中心】${this.$moment(date.from).format('YYYY年MM月DD日')}到${this.$moment(date.now).format('YYYY年MM月DD日')}，发稿（${_data.AllCountPublish}）篇，大祥融媒${_data['原创'].count}篇,大祥之窗上稿${_data.countPublish}篇，其中原创${_data['原创'].count}篇，大祥发布${_data.DXFB_Count}篇，转载${_data['转载'].count}篇，向市以上各大媒体发稿${_data.MtJJ_Count}篇。`
         console.log(result)
         this.datalist=[]
         console.log(_data)
...
a:visited {background:#FFFF85;  text-decoration:underline;
           color:cadetblue;} /* 已被访问的链接 */
a:hover {background:#129416;text-transform:uppercase;  text-decoration:underline;color:#fff;}   /* 鼠标指针移动到链接上 */
a:active {background:#129416;  text-decoration:underline;color:#fff;}  /* 正在被点击的链接 */
